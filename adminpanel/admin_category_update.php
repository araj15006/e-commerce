...
$category_name = '';
$category_name_query = "select * from category where `id`='$id'";
$category_name_run = mysqli_query($conn,$category_name_query);
$category_name_fetch = mysqli_fetch_assoc($category_name_run);
 
if(empty($category_name_fetch)== false){
  $category_name=$category_name_fetch['heading'];
}
...
if(isset($_POST['title'])){    
      ...
      $image_upload_query = "UPDATE `category` SET `heading`='$title',`image_name`='$file_name',`status`='status' WHERE `id` = $id ";
    ...
    <?  }?>
        ...
        <form accept-charset="UTF-8" action="" method="POST" enctype="multipart/form-data">
          <div class="form-group">
            ...
            <input type="text" name="title" class="form-control" value="<?= $category_name ?>"  placeholder="Enter title">
          </div>
          <hr>
